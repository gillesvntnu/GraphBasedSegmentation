...
class kpts_decoder(nn.Module):
    ...
    def create_graph(self, num_nodes: int) -> np.ndarray:
        adjacency = []
        adjacency_row = list(range(num_nodes))
        for ii in range(num_nodes):
            adjacency.append(adjacency_row)
            adjacency_row = adjacency_row[1:] + adjacency_row[:1] # rotate by 1
...
if __name__ == '__main__':
    ...
    m = CNN_GCN(kpt_channels=2, gcn_channels=[16, 32, 32, 48], backbone='resnet50', num_kpts=num_kpts, is_gpu=gpu)
    ...
    if gpu:
        ...
        kpts = kpts.cuda()
    o = m(img)
    loss = load_loss("L2", device=gpu, class_weights=None)
    print(loss(o, kpts))
